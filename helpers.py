...
def WriteFile(g : GLOBALS, text : str):
    ...
    output = open(g.outputFile, 'w')

    if (g.flags.fliteHTSFlag != 0):
        text = text[:-1]
        print(f"phones : {text}")
        output.write(text)
        output.close()
    ...
    output.close()
    if (g.flags.writeFormat == 0 and g.flags.pruiningFlag != 0):
...
def CombinationCorrection(g : GLOBALS, phone : str, left : str, right : str, isReverse : int) -> str:
    ...
    while g.combvars.leftStr[i] != '':
        if isReverse == 1:
            phonecopy = phonecopy.replace(g.combvars.rightStr[i], "!")
            phonecopy = phonecopy.replace("!", g.combvars.leftStr[i])
        else:
            phonecopy = phonecopy.replace(g.combvars.leftStr[i], "!")
            phonecopy = phonecopy.replace("!", g.combvars.rightStr[i])
...
def SplitSyllables(g : GLOBALS, input : str) -> int:
    ...
    if ln > 1:
        for i in range(ln-1,-1,-1):
            ...
            if i-1 > 0:
                ...
                g.syllableList[i-1] = g.syllableList[i-1].replace("&&","&")
        ...
        if (CheckVowel(g.syllableList[0],1,0) == 0 and g.flags.writeFormat != 3) or Checkeuv(g.syllableList[0]) == 1:
            g.syllableList[0] += '&av'
    ...
    for i in range(ln):
        ...
        if g.syllableList[i].find('&&') != -1:
            g.syllableList[i] = g.syllableList[i].replace('&&', '&')
...
def WritetoFiles(g : GLOBALS) -> int:
    ...
    # //syllable
        ...
        syllablesPrint = 0
        for i in range(g.syllableCount):
            g.syllableList[i] = g.syllableList[i].replace('euv', 'eu')
